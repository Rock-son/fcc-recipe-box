...
export function changeLocalStorageData(cuisineType, recipeIndex, newValue) {
    LOCALSTORAGE_DATA[cuisineType][recipeIndex] = newValue;
}

export function deleteLocalStorageData(cuisineType, recipeIndex) {
    delete LOCALSTORAGE_DATA[cuisineType][recipeIndex];
}

export function setRecipeIndex(newIndex) {
    RECIPE_INDEX = newIndex;
}

export function setCuisineType(newType) { 
    CUISINE_TYPE = newType;
}

export function setOpenedRecipeBox(newBox) { 
    OPENED_RECIPEBOX = newBox;
}